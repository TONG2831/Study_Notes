...
`--global`也可以使用`--system` 替代，`--global` 表示当前系统用户下的所有Git仓库使用该配置，`--system` 表示所有系统用户下都使用该配置。
...
在当前的分支上做的工作没有完成，不能提交，但是也不能丢失进度，这个时候可以使用该指令保存工作进度。然后切换到其他的分支（没有保存的工作现场是不能够切换分支的）。
